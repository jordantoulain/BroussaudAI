...
| **Gestion des sessions** | Affichage des appareils connectés avec device_info, dates de création/expiration. Déconnexion forcée d'une session. Vérification que la session appartient à l'utilisateur. | `api/app/api/routes/sessions.py`, `web/src/components/chat/SessionsList.jsx`, `web/src/components/chat/UserProfile.jsx`, `web/src/components/ui/SideCanvas.jsx` | `GET /sessions/`, `DELETE /sessions/{session_id}`, `SessionsList`, `SideCanvas`, `UserProfile` | C1.2, C2.2, C2.3 | ⭐⭐⭐ | Sécurité des sessions, UX |
